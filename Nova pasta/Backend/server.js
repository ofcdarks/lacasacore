...
app.post('/api/analyze/thumbnail', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (customPrompt && customPrompt.trim()) {
            ...
            - **IDEIA 1 (RÉPLICA APRIMORADA DA ORIGINAL):** Replique a thumbnail de referência quase 1:1. Mantenha exatamente a mesma composição, ângulo de câmera, posição de personagens, paleta de cores, quantidade de texto e posicionamento do texto. Apenas eleve a qualidade: mais nitidez, contraste, iluminação cinematográfica, correções de cor profissionais e micro ajustes de limpeza. Não altere o storytelling principal, apenas entregue a versão definitiva, com acabamento premium e pequenos upgrades que deixem a versão praticamente igual, porém mais clara e clicável.
            - **IDEIA 2 (TURBO CTR INOVADOR):** Crie um conceito completamente novo, hiper focado em CTR alto (30%+). Pode mudar cenário, personagens, enquadramento e paleta, explorando um novo gancho visual com FOMO extremo, contraste máximo, expressões dramáticas e elementos que não existem na thumb original. O objetivo é ser agressivo em curiosidade, urgência e clareza da promessa visual.
        ...
        `;
            ...
            - **IDEIA 1 (RÉPLICA APRIMORADA DA ORIGINAL):** 
              * Replique a estrutura da thumbnail de referência quase 1:1: mantenha ângulo de câmera, enquadramento, posição dos personagens/objetos, quantidade e posição do texto, paleta de cores e elementos de cenário.
              * Preserve exatamente o storytelling visual, apenas elevando a qualidade (recortes perfeitos, tratamento de pele profissional, brilho nos olhos, textura realista, correção de cor cinematográfica).
              * Ajustes permitidos: aumentar nitidez, reforçar contraste, limpar ruídos, adicionar luzes/sombras profissionais e aplicar efeitos de texto Photoshop com valores específicos (stroke, drop shadow, outer glow, bevel & emboss).
              * Resultado deve ser praticamente igual à thumb original, mas com sensação de upgrade premium e leitura instantânea.
            
            - **IDEIA 2 (TURBO CTR INOVADOR):** 
              * Crie um conceito totalmente novo com foco em CTR 30%+: novo enquadramento, nova composição, novos elementos que gerem curiosidade extrema.
...
app.post('/api/script-agents/:agentId/generate', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (requestedParts && requestedParts !== idealParts) {
            console.warn(`[Roteiro] Partes solicitadas (${requestedParts}) divergentes da estrutura ideal (${idealParts}). Ajustando para manter a minutagem.`);
